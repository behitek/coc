...
export const CurrentWarPage: React.FC<CurrentWarProps> = ({ clanTag, onMemberClick }) => {
  ...
  useEffect(() => {
    ...
    fetchWar();
    const interval = setInterval(fetchWar, 60000); // Auto-refresh every minute
      ...
      <div className={`bg-slate-800 rounded-xl p-6 shadow-lg relative overflow-hidden border-2 transition-all ${isPerfectWar ? 'border-coc-gold shadow-2xl shadow-yellow-600/30' : 'border-slate-700'}`}>
         ...
         <div className="flex flex-col md:flex-row justify-between items-center relative z-10">
            ...
            <div className="flex flex-col items-center mx-8 my-4 md:my-0">
                ...
                <div className="flex items-center bg-slate-900/80 px-4 py-2 rounded-lg border border-slate-600 backdrop-blur-sm">
                    <Timer className="text-coc-gold w-5 h-5 mr-2" />
                    <div className="text-center">
                        <div className="text-xs text-slate-400 uppercase tracking-wider">{timerLabel}</div>
                        <div className="text-xl font-mono font-bold text-white w-32">{timer}</div>
                    </div>
                </div>
